...
void ActivateTera(u32 battler)
{
    ...
    if (FlagGet(FLAG_TERA_ORB_CHARGED) != 0
        && (FlagGet(FLAG_TERA_ORB_NO_COST) == 0 || !FlagGet(FLAG_TERA_ORB_NO_COST))
        && side == B_SIDE_PLAYER
...
bool32 CanTerastallize(u32 battler)
{
    ...
    else if (FlagGet(FLAG_TERA_ORB_NO_COST))
    {
        // Tera Orb is not depleted, go to HasTrainerUsedGimmick
    }
    else if (!FlagGet(FLAG_TERA_ORB_CHARGED))
    {
        return FALSE;
    }
...
{
    return GetMonData(&GetBattlerParty(battler)[gBattlerPartyIndexes[battler]], MON_DATA_TERA_TYPE, NULL);
}
